...
static void check_and_free_timer_in_array(struct bpf_array *array, void *val)
{
	if (unlikely(map_value_has_timer(&array->map)))
		bpf_timer_cancel_and_free(val + array->map.timer_off);
}
